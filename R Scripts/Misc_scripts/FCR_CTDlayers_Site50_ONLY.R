...
#save layer 1 as csv
write.csv(layer1, "FCR_CTD_50surf.csv", row.names=FALSE)
write.csv(layer13, "FCR_CTD_50_9m.csv", row.names=FALSE)
...
fcr50_layers <- df.final[order(df.final$Date, df.final$Depth_m), ]
 # arrange(df.final, Date)

write.csv(fcr50_layers, "FCR_50_layersALL.csv", row.names=FALSE)
