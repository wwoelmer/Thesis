...
sites <- read_csv("./Materials for R mapping/Jun_RCC_CO2_CH4_chla_mapping.csv")
...
b = a + 
  ...
  geom_point(data = sites, aes(x = lon, y = lat, cex = ch4_uM), pch = 21, bg = "magenta", col = "black")
...
e = ggplot(fcr_map, aes(long, lat, fill=Depths)) +
  geom_polygon()+
  geom_path(color="black", lwd = 1.7) +
  coord_equal() +
  #geom_point(data = traps, aes(x = lon, y = lat), pch = 21, bg = "red", col = "black", cex = 5)+
