...
sites <- read_csv("./Materials for R mapping/nutrient_mapping/08_August/Aug_RCC_ammonium_mapping.csv")
...
b = a +
  ...
  geom_point(data = sites, aes(x = lon, y = lat, size = NH4_ppb ), pch = 21, bg = "orange", col = "black")+
...
c = c + ggtitle('August') + theme(plot.title = element_text(size = 40))


pdf("./Materials for R mapping/nutrient_mapping/08_August/22Aug19_RCC_NH4.pdf", width = 8, height = 6)
c
dev.off()

png("./Materials for R mapping/nutrient_mapping/08_August/22Aug19_RCC_NH4.png", width = 1100, height = 800)
c
dev.off()
